  ...
  def create_and_put_pdf(response_set_code, mcoc_renewal_id, grantee_name, project_name, user_id)
    ...
    @mcoc_renewal = McocRenewal.find(mcoc_renewal_id)
     ...
     if @response_set
       ...
       FinishedSurveyMailer.delay.send_finished_email(@finished_survey) #delayed_job
       #FinishedSurveyMailer.send_finished_email(@finished_survey).deliver #when we're not delaying...
